...
HOMR_API_PATH = ROOT_DIR / "homr-service" / "homr" / "homr_api.py"


def _load_homr_api_module():
    # load the homr helper file directly from its path.
    spec = importlib.util.spec_from_file_location("tably_homr_api", HOMR_API_PATH)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load homr_api from {HOMR_API_PATH}")
...
homr_api = _load_homr_api_module()

ensure_homr_models_ready = homr_api.ensure_homr_models_ready
IMAGE_EXTENSIONS = homr_api.IMAGE_EXTENSIONS
convert_image_to_musicxml = homr_api.convert_image_to_musicxml
is_supported_image_extension = homr_api.is_supported_image_extension
